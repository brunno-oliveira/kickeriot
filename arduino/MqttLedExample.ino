 ...
 * Versão 0.1
 */
#include <stdio.h> include <string.h> include <SPI.h> include <Ethernet.h> include <MQTTClient.h>
byte mac[] = { 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED }; byte ip[] = { 192, 168, 1, 177 }; EthernetClient 
net; MQTTClient client; int Red, Green, Yellow; void setup() {
  Serial.begin(9600);
...
void connect() {
  ...
  Serial.println("\nconnected!");
  client.subscribe("/example/leds/#",[1]);
}
void loop() {
  client.loop();
  if (!client.connected()) {
...
//Callback Method void messageReceived(String topic, String payload, char * bytes, unsigned int length) {
...
//Função que recebe o Topico e a mensagem void controller(String topic, String payload) {
  String sTopicos[4];
...
//Extrai os Topicos void getTopics(String sTopicoOriginal, String sTopicos[]) {
  int count = 1;
